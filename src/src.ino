...
static const uint32_t CLASSES=2;               // HUNCH2
...
// ================== HUNCH2 labels ==================
static const char* HUNCH2[CLASSES] = {
  "good","bad"
};
...
void loop(){
  ...
  int rc = predict(gInput, inputShape, 4, &out_raw);   // OUT: [2]
  ...
  if (streaming()) {
    ...
    // log testuale locale
    const char* name = (cls>=0 && cls<(int)CLASSES) ? HUNCH2[cls] : "unknown";
    Serial.print("[ZANT] HUNCH2 top1: idx=");
    Serial.print(cls);
    Serial.print(" label=");
    Serial.print(name);
    Serial.print(" prob=");
